...
if( $ARGV[0] != 1 ) {
 $residual = (split(' ', ` grep ' residual' BATCH/$families[$i]/step14.out ` ))[2];
}
...
my @mu;
if( $ARGV[0] != 1 ) {
 @mu = split(' ', ` grep -m 1 -A 1 'mu' BATCH/$families[$i]/step14.out | tail -n 1 ` );
}
...
my @tree = ` grep '___' BATCH/$families[$i]/step8.out ` ;
